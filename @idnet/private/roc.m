function fig_hl = roc(boolNets, groundTruth)
% ROC is to plot the ROC curve in terms of the given variable (e.g.
%   model orders, labmdas).
...
% figure('Visible', 'on');
% plot(PRpts(:,1), PRpts(:,2), '-bo');
% hold on;
% plot([min(PRpts(:,1)) max(PRpts(:,1))], [-min(PRpts(:,1))+1 -max(PRpts(:,1)+1)], '-.');
% hold off;
% title('Precision-Recall Curve for Model Orders');
% xlabel('Recall = TP/(TP+FN)');
% ylabel('Precision = TP/(TP+FP)');

% Fig 3. ROC Curve in terms of model orders
fig_hl = figure('Visible', 'on');
% plot(ROCpts(:,1), ROCpts(:,2), '-s');
stairs(ROCpts(:,1), ROCpts(:,2), '-s');
hold on;
plot([min(ROCpts(:,1)) max(ROCpts(:,1))], ...
     [min(ROCpts(:,1)) max(ROCpts(:,1))], '-.', ...
     'color', [.5 .5 .5]);
hold off;
xlim([0 1]); ylim([0 1]);
